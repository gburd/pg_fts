...
| R8 | **Codeberg Actions is not running.** Either enable Actions for the repository in Codeberg's settings (account-side, one switch) and confirm a run reaches `started_at`, or delete `.forgejo/workflows/` and every "Forgejo CI" reference (RELEASING.md step 3, the flake comment) so the repo stops describing a pipeline it does not have. Do not leave it in the current state, where it looks like a second gate and is nothing. | **open -- needs an account-side action** |
