...
- **Instance**: EC2 **r7i.4xlarge** (16 vCPU Sapphire Rapids, 123 GiB), Fedora
  Cloud 43, us-east-2. Instance id `REDACTED-INSTANCE`, owner=gburd-agent,
...
- First instance `REDACTED-INSTANCE` was terminated (Fedora's 5 GB root can't
  hold the toolchain; an attempt to overlay /usr onto /data wedged sudo). Relaunched
  with a 300 GB **root** volume (`REDACTED-INSTANCE`), which Fedora auto-grows
...
Instance `REDACTED-INSTANCE` (and the earlier `REDACTED-INSTANCE`) plus their
